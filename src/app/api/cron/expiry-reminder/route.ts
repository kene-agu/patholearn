...
function trialEmailHtml(email: string, daysLeft: number, trialEndDate: string): string {
  const date = new Date(trialEndDate).toLocaleDateString("en-GB", {
    day: "numeric", month: "long", year: "numeric",
  });
  const urgency = daysLeft === 1 ? "tomorrow" : `in ${daysLeft} days`;
    ...
    <tr><td align="center">
      <table width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;">
        ...
        <tr><td style="background:#ffffff;border-radius:16px;border:1px solid #e2e8f0;padding:36px 32px;">
          ...
          <p style="margin:0 0 8px;font-size:13px;font-weight:600;color:#f59e0b;text-transform:uppercase;letter-spacing:.05em;">
            Free trial ending soon
          </p>
          <h1 style="margin:0 0 16px;font-size:22px;font-weight:700;color:#0f172a;line-height:1.3;">
            Your free trial expires ${urgency}
          </h1>
          <p style="margin:0 0 24px;font-size:15px;color:#475569;line-height:1.6;">
            Your 14-day free trial ends on <strong>${date}</strong>.
            Subscribe now to keep access to AI-powered slide analysis and all the tools you've been using.
          </p>
          ...
          <table cellpadding="0" cellspacing="0" width="100%">
            <tr><td align="center" style="padding-bottom:24px;">
              <a href="${APP_URL}/pricing"
                style="display:inline-block;background:linear-gradient(135deg,#f59e0b,#f97316);color:#ffffff;font-weight:700;font-size:15px;text-decoration:none;padding:14px 32px;border-radius:10px;">
          ...
          <!-- Features reminder -->
          <table cellpadding="0" cellspacing="0" width="100%" style="background:#fffbeb;border-radius:10px;padding:16px;">
            <tr><td>
              <p style="margin:0 0 10px;font-size:12px;font-weight:600;color:#92400e;text-transform:uppercase;letter-spacing:.05em;">What you'll lose when your trial ends</p>
              ${["Unlimited AI slide analysis", "Cross-device spaced repetition", "PDF export of every analysis", "Full progress & confidence tracking", "Save cases to My Cases"].map(f =>
                `<p style="margin:0 0 6px;font-size:14px;color:#78350f;">✗ &nbsp;${f}</p>`
              ).join("")}
            </td></tr>
          </table>
        ...
        <tr><td style="padding:24px 0 0;text-align:center;">
          <p style="margin:0;font-size:12px;color:#94a3b8;">
            You're receiving this because you signed up for a free trial of PathoLearn.<br>
            Questions? Reply to this email.
          </p>
        </td></tr>
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // ── 1. Paid subscriptions expiring in 1–3 days ──────────────────────
    ...
    // ── 2. Free trials expiring in 1–3 days ─────────────────────────────
    // trial_end = trial_started_at + 14 days
    // trial_end in [now+1d, now+3d]  →  trial_started_at in [now-13d, now-11d]
    const minus11 = new Date(now); minus11.setDate(minus11.getDate() - 11);
    const minus13 = new Date(now); minus13.setDate(minus13.getDate() - 13);
    const minus11s = minus11.toISOString().slice(0, 10);
    const minus13s = minus13.toISOString().slice(0, 10);

    const { data: trialProfiles, error: trialError } = await supabaseAdmin
      .from("profiles")
      .select("id, trial_started_at")
      .eq("subscription_status", "trialing")
      .gte("trial_started_at", `${minus13s}T00:00:00Z`)
      .lte("trial_started_at", `${minus11s}T23:59:59Z`);
    ...
    for (const profile of trialProfiles ?? []) {
      ...
      const trialEnd = new Date(new Date(profile.trial_started_at).getTime() + 14 * 24 * 60 * 60 * 1000);
      ...
      try {
        await resend.emails.send({
          ...
          to:      email,
          subject: daysLeft === 1
            ? "⏰ Your PathoLearn free trial expires tomorrow"
            : `Your PathoLearn free trial expires in ${daysLeft} days`,
          html: trialEmailHtml(email, daysLeft, trialEnd.toISOString()),
        });
        sent++;
      } catch (emailErr) {
        errors.push(`${email}: ${emailErr}`);
    ...
    if (sent === 0 && errors.length === 0) {
      return NextResponse.json({ sent: 0, message: "No expiring subscriptions or trials today" });
    }
