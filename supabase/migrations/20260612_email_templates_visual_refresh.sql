...
-- Unified design: soft purple-tinted body background, white card with
-- subtle drop shadow, gradient hero band at the top of each card with
-- LABEL + HEADING in white, content section below with body, CTA, and
-- (depending on the kind) a features list or receipt-style box.
--
-- ⚠️  Upserts subject and html for all seven kinds — overwrites any
-- admin edits previously made via /admin/Emails. Safe to run on a fresh
-- env or one where nobody has hand-edited the templates yet.
